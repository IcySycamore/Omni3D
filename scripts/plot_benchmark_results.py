...
def plot_cdf(errors, label, axis):
    """绘制累积分布曲线。"""
    sorted_errors = np.sort(errors)
    y = np.arange(1, len(sorted_errors) + 1) / len(sorted_errors)
    axis.plot(sorted_errors, y, label=label)
...
def _plot_pose(data, prefix, out_dir):
    """绘制位姿评测相关图表。"""
    ...
    plot_bar(
        [s["ate"] for s in data],
        scenes,
        f"{prefix}: ATE per scene",
        "ATE",
        out_dir / f"{prefix}_ate.png",
    )
    plot_bar(
        [s["auc"] for s in data],
        scenes,
        f"{prefix}: AUC per scene",
        "AUC",
        out_dir / f"{prefix}_auc.png",
    )


def _plot_recon(data, prefix, out_dir):
    """绘制重建评测相关图表。"""
    fig, axis = plt.subplots(figsize=(8, 5))
    scenes = [s["scene"] for s in data]
    ...
    width = 0.35
    axis.bar(x - width / 2, acc, width, label="Accuracy")
    axis.bar(x + width / 2, comp, width, label="Completion")
    axis.set_xticks(x)
    axis.set_xticklabels(scenes, rotation=45, ha="right")
    axis.set_ylabel("Distance")
    axis.set_title(f"{prefix}: Accuracy vs Completion")
    axis.legend()
    axis.grid(True, axis="y", linestyle="--", alpha=0.5)
    fig.tight_layout()
...
def main() -> None:
    ...
    if "rel_rotation_mean_deg" in data[0]:
        _plot_pose(data, prefix, out_dir)

    if "accuracy_mean" in data[0]:
        _plot_recon(data, prefix, out_dir)
