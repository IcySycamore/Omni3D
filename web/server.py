...
# ---- 重建 ----
# PLY 走**二进制小端**：同样 100 万点，ASCII 约 80MB，二进制 27B/点 约 27MB。
# 坐标用 float64（header 写 `property double`）：内部变换本来就是 float64，
# 落盘再降回 float32 属于无谓的二次舍入（#24）。颜色仍是 uchar，只占 3B/点。
# 注：float32 **不是**当前误差瓶颈（相对精度 ~1e-7 而自洽性误差 ~2.5%），
# 这里只是「既然不要速度，就顺手不留无谓损失」。
_PLY_STRUCT = np.dtype([
    ("x", "<f8"), ("y", "<f8"), ("z", "<f8"),
    ("red", "u1"), ("green", "u1"), ("blue", "u1"),
])
# 与 `_PLY_STRUCT` 一一对应 —— 改结构体必须同步改这里，否则 PLY 读不出来。
_PLY_HEADER_PROPS = (
    "property double x\nproperty double y\nproperty double z\n"
...
def _sample_for_render(points, colors, max_render_points=None):
    ...
    # 渲染子集特意降回 float32：屏幕分辨率远低于 float32 精度，用 double 白占内存（#24）
