...
| 角色            | 是什么                                            | 位置             |
| --------------- | ------------------------------------------------- | ---------------- |
| **网页 client** | 真正的客户端实现（在浏览器里跑）                  | `web/index.html` |
| **桌面 client** | 另一种客户端实现（PyQt5 + VTK）                   | `desktop/`       |
| **移动端 App**  | **不是第三种 client**：网页的 WebView 壳 + 本地桥 | `qt_app/`        |

**硬约束：**

1. **server 只有一种。** 所有重建能力只在服务器实现一次。
2. **网页 client 与桌面 client 都只是 server 的 client。** 任何重建 / 尺度 / 历史逻辑，
   都不允许在 client 里重复实现；client 只负责「采集」与「呈现」。
3. **移动端 App 不实现独立 UI**（它就是网页 client 的壳）。新增功能优先做在网页/client 侧，
   不要在 App 里另起一套界面。
...
| **桌面客户端**        | PyQt5 + VTK 的桌面 client（登录窗 ⚙ 配置服务器）                          | `desktop/`                                         |
| **账号**              | 用户名 + salt + verifier；明文密码不落库                                  | `web/auth_store.py`                                |
| **用户名规则**        | 3–32 字符、仅 `[A-Za-z0-9_.-]`；**服务端强制**                            | `web/auth_store.validate_username`                 |
| **密码规则**          | ≥ 8 位且非全空白；**只能在客户端校验**（协议不上行明文密码）              | `web/index.html` / `desktop/api_client.py`         |
| **verifier**          | `sha256(salt + password)`，注册时由**客户端**计算上行                     | `web/auth_store.py` / `desktop/api_client.py`      |
| **nonce / proof**     | 登录挑战-应答：`proof = sha256(nonce + verifier)`，nonce 一次性           | 同上                                               |
| **token**             | 登录成功后签发的会话令牌，请求经 `X-Auth-Token` 头携带                    | `web/session_manager.py`、`desktop/session.py`     |
...
```
Omni3D/
├── web/          # server（FastAPI）+ 前端 index.html（web client）
...
- 桌面端 Qt 已**复活**为主路径 `desktop/`（PyQt5 + VTK），不再有 `legacy/`
